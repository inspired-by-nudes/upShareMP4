    ...
    if INFERENCE_TEXT_MODEL:
        try:
            ...
            if res.status_code == 200:
                ...
                return result.replace('```html', '').replace('```', '').strip(), "🤖 Ollama"
...
def process_yt_dlp(url: str, user_id: str, task_id: str, expire_days: int, force_article: bool = False):
    ...
    try:
        ...
        if media_files:
            ...
            # Handle Image Carousel (Instagram)
            new_id = generate_secure_id()
            html_path = os.path.join(DOWNLOAD_DIR, f"{new_id}.html")
            ...
            for idx, img_f in enumerate(sorted(image_files)):
                ...
                img_tags += f"<img src='/videos/{new_img_name}' style='width:100%; scroll-snap-align: center; flex: 0 0 100%; object-fit: contain;'>"
            ...
            <style>
                ...
                .carousel {{ display: flex; overflow-x: auto; scroll-snap-type: x mandatory; width: 100vw; height: 100vh; }}
                .carousel::-webkit-scrollbar {{ display: none; }}
            </style>
            </head><body>
                <div class="carousel">{img_tags}</div>
            </body></html>
            """
